...
void APIC::promoteEvent(APIC apic) {
	...
	do {//exits only when input is "c" or "ss"
		cout << endl << "Do you want to promote a conference or a summer school " << endl;
	...
	if (pick == "c") {
		//prints conferences
		printEventsC();
		do { //exits only when input is a valid number
		...
		//promotes selected user
		for (unsigned int i = 0; i < eventsC.size(); i++) {
			if (i + 1 == pick2) {
				eventsC[i].insertPromoter(apic.getUserLogged());
				//re-writes conference file with uptaded info
				remove("eventsC.txt");
	...
	if (pick == "ss") {
		...
		for (unsigned int i = 0; i < eventsSS.size(); i++) {
			if (i + 1 == pick2) {
				...
				//re-write summer schools file with updated info
				...
				for (unsigned int i = 0; i < eventsSS.size(); i++) {
					...
					if (i + 1 != pick2) {
						...
						//prints all formers for each user
						for (unsigned int j = 0; j < eventsSS[i].getFormers().size(); j++) {
							//if it is the last element ends with ','
							if (j + 1 == eventsSS[i].getFormers().size())
								eventsSSFile << eventsSS[i].getFormers()[j].getLoginName() << "," << endl;
							//other elements end with ';'
							 else
								eventsSSFile << eventsSS[i].getFormers()[j].getLoginName() << ";";
							}
					}
					//lines of the user selected
					if (i + 1 == pick2) {
						eventsSSFile
								<< eventsSS[i].getUserCreated().getLoginName()
								<< ";" << eventsSS[i].getLocal() << ";"
						...
						//inserts the user logged as the promoter
						eventsSSFile << apic.getUserLogged().getLoginName() << endl;
					}
				}
			}
		}
...
void APIC::createSummerSchool(APIC apic) {
	...
	EventSummerSchool newSummerSchool(apic.getUserLogged(), local, title, date, area, 0, formers);
...
void APIC::createConference(APIC apic) {
	...
	EventConference newConference(apic.getUserLogged(), local, title, date, area, 0, numberPeople);
	...
	if (areaString == "all") {
		...
		for (unsigned int i = 0; i < areasS.size(); i++) {
			for (unsigned int j = 0; j < users.size(); j++) {
				find = 0;
				for (unsigned int k = 0; k < users[j].getVectorAreas().size(); k++) {
...
EventSummerSchool APIC::readSummerSchool(stringstream & s) {
	string newUserString, newDateString, newLocal, newTitle, newArea, newSupportString, newFormersString;
	int newSupport;
	User newUser;
	...
	EventSummerSchool newSS(newUser, newLocal, newTitle, newDate, newArea, newSupport, newFormers);
	return newSS;
}

EventConference APIC::readConference(stringstream & s) {
	string newUserString, newDateString, newLocal, newTitle, numberString, newArea, newSupportString;
	...
	EventConference newC(newUser, newLocal, newTitle, newDate, newArea, newSupport, newNumber);
	return newC;
}
